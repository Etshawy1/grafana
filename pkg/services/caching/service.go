...
func (s *OSSCachingService) updateCacheFunction(ctx context.Context, res *backend.QueryDataResponse) {
	...
	queryCachingTTL := s.getQueryCachingTTLFromRequest(req)
	if queryCachingTTL != 0 {
		backend.Logger.Info("Caching QueryDataResponse", "cacheKey: ", cacheKey, "queryCachingTTL in ms: ", queryCachingTTL)
		s.cache.Set(ctx, cacheKey, b.Bytes(), time.Duration(queryCachingTTL*1000000))
	}
}

func (s *OSSCachingService) getCacheKeyFromRequest(req *backend.QueryDataRequest) string {
	return req.Headers["http_X-Dashboard-Uid"] + "_" + req.Headers["http_X-Datasource-Uid"] + "_" + req.Headers["http_X-Grafana-Org-Id"] + "_" + req.Headers["http_X-Panel-Id"] + "_" + req.Queries[0].TimeRange.To.Sub(req.Queries[0].TimeRange.From).Round(time.Minute).String()
}
